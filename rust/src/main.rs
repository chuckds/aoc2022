...
fn json_to_aocresult(json_val: &json::JsonValue) -> AoCResult {
    match json_val {
        ...
        json::JsonValue::Array(vec) => AoCResult::NumVec(vec.iter().map(|x| x.as_i64().unwrap()).collect()),
        ...
        for test_info in tests {
            if let json::JsonValue::Array(ref test_info) = test_info {
                if let json::JsonValue::Array(ref results) = test_info[3] {
                    test_hash.entry(test_info[0].as_str().unwrap().to_string())
                        .or_insert(Vec::new())
                        .push((test_info[2].as_str().unwrap().to_string(), (json_to_aocresult(&results[0]), json_to_aocresult(&results[1]))));
                }
            }
        }
    }
...
fn day_test_json(
    ...
    for (input_file, expected_result) in test_hash_json.get(day).expect("Can't find day in hash") {
        ...
        match expected_result {
            (AoCResult::Number(n1), AoCResult::Number(n2)) => assert_eq!((*n1, *n2), test_func(&input_path)),
